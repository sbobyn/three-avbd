...
| 9 npm library (v0.1) | built 2026-09-26, not published: `src/lib` (`World`, `Body`, `Joint`, `BodyMesh`; `three-avbd/advanced`) over the 3D GPU solver, y-up (the solver's gravity now pulls against an `up` parameter; reference scenes keep z). Runtime adds, removal with slot reuse, breakable joints with `onBreak`, async readback, zero-copy drawing. GPU tests on the public API; `examples/basic`; `pnpm build:lib` packs 139 kB, verified installed in a blank Vite project (TS 5.9 and 7). Design: `docs/API.md`. |
