...
# crop on the painting and no text (the post says what it is); --captions adds a hook, and
# --tag a "recorded live" note.
#
#   python3 tools/trailer/starry.py [--captions] [--tag] [--machine "Apple M4 Max"] [--out starry-night.mp4]
...
args.add_argument('--captions', action='store_true', help='add the hook captions')
args.add_argument('--tag', action='store_true', help='add the "recorded live" tag')
...
for i, (s, x, y, size, t0, t1, color) in enumerate(CAPTIONS if args.captions else []):
    f.append(draw(f'starry{i}', s, x, y, size, t0, t1, color))
if args.tag:
    f.append(draw('starrytag', TAG, 'w-tw-48', 'h-62', 22, -1, 40, MUTED))
cmd = ['ffmpeg', '-v', 'error', '-y', '-i', os.path.join(CLIPS, 'starry.mp4'), '-t', '36.2', '-vf', ','.join(f),
