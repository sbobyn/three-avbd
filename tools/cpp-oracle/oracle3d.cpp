// Headless driver for the upstream avbd-demo3d solver (reference/avbd-demo3d). Steps a scene
// and prints body poses as JSON so the TypeScript port can be diffed against the original.
// Usage: oracle3d <sceneIndex> <frame,frame,...>   (samples the given frames, ascending)
#include <cstdio>
#include <cstdlib>
#include <set>
#include <sstream>
...
{
    if (argc < 3) { fprintf(stderr, "usage: oracle3d <scene> <frame,frame,...>\n"); return 1; }
    ...
    for (int f = 0; f <= frames; f++)
    {
        if (sampleFrames.count(f))
        {
            ...
            for (size_t i = 0; i < bodies.size(); i++)
            {
                ...
                printf("%s[%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g]", i ? "," : "",
